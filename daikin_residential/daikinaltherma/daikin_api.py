...
from .sensor import createSensorsByDaikinAPI

from .daikin_base import Appliance

from .const import DAIKIN_API, DOMAIN, DAIKIN_DEVICES, CONF_TOKENSET
...
class DaikinApi:
    ...
    async def refreshAccessToken(self):
        ...
        try:
            await self.retrieveAccessToken(self._daikinuser, self._daikinpassword)
            data[CONF_TOKENSET] = self.tokenSet
        except Exception:
            raise Exception(
    ...
    async def getCloudDevices(self):
        ...
        for dev_data in self.json_data or []:
            ...
            _LOGGER.info(
                "Found device '%s' with gateway model '%s'", device_model, gateway_model
            )
            ...
            if device_model in ("Altherma", "NDJ"):
                ...
                _LOGGER.info(
                    "Device '%s' with gateway model '%s' is filtered out because it is not a supported model",
                    device_model,
                    gateway_model,
                )
        self.data[DOMAIN] = {DAIKIN_API: self, DAIKIN_DEVICES: res}
        return res
    ...
    async def async_update(self, **kwargs):
        ...
        for dev_data in self.json_data or []:
            if "id" in dev_data:
                ...
                if dev_data["id"] in self.data[DOMAIN][DAIKIN_DEVICES]:
                    self.data[DOMAIN][DAIKIN_DEVICES][dev_data["id"]].setJsonData(
                        dev_data
                    )
